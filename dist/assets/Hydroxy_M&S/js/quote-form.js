...
class QuoteForm {
    ...
    init() {
        // Usuwamy obsługę zdarzenia submit, ponieważ FormSubmit.co zajmie się wysyłką
        // Zostawiamy tylko walidację
        this.addRealTimeValidation();
        this.addErrorContainers();
        
        // Dodajemy walidację przed wysłaniem
        this.form.addEventListener('submit', this.handleSubmitValidation.bind(this));
    }
    ...
    handleSubmitValidation(e) {
        ...
        // Jeśli walidacja przeszła, formularz zostanie wysłany do FormSubmit.co
    ...
    async handleSubmit(e) {
        ...
        try {
            ...
            const response = await fetch('quote_handler.php', {
                method: 'POST',
                body: formData,
                headers: {
    ...
    showMessage(message, type) {
        if (this.messageContainer) {
            ...
            // Style dla różnych typów wiadomości
            if (type === 'success') {
                this.messageContainer.style.cssText = `
                    padding: 10px;
                    border-radius: 4px;
    ...
    setLoading(loading) {
        if (loading) {
            ...
            if (this.btnText) {
                this.btnText.textContent = 'Request Quote';
            }
        }
    }
}
...
document.addEventListener('DOMContentLoaded', function() {
    const quoteForm = new QuoteForm('#quote-form');
});